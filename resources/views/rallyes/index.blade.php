            ...
            <table class="table dataTable js-exportable">
              ...
              <tbody>
                ...
                <tr class="table-success">
                  ...
                  <td></td>
                  @endif
                  <td><strong>{{$rallye->rallyemail}}</strong></td>
                  @if($rallye->status)
                  ...
                  @endif
                  <td>
                    <a href="/rallyes/{{$rallye->id}}/reverseStatusById"><button type="button"
                        class="btn btn-info btn-sm"><span class="glyphicon glyphicon-flash"></span></button></a>
                    <a href="/rallyes/{{$rallye->id}}"><button type="button" class="btn btn-warning btn-sm"><span
                          class="glyphicon glyphicon-edit"></span></button></a>
                    <button type="button" class="btn btn-danger btn-sm  " data-toggle="modal"
                      data-target="#deleteRallyeModal_{{$rallye->id}}"><span class="glyphicon glyphicon-remove"></span></button>
                  </td>
                  <!-- +AJO : Model section begin -->
                  <div class="modal fade" id="deleteRallyeModal_{{$rallye->id}}" tabindex="-1" role="dialog">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content bg-danger">
                        ...
                        <div class="modal-body col-white">
                          {{ Form::open(['method' => 'GET', 'url' => route('rallyes.destroy', $rallye->id)]) }}
                          @csrf
                          {{ method_field('DELETE') }}
                          <p><b>Rallye: </b>{{$rallye->title}}</p>  
                                  
                          <div class="form-group">
                              <label for="action"><b>Are you sure you want to delete this rallye ?</b></label>
                          </div>
                        </div>
